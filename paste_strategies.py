...
class XdotoolCtrlShiftVStrategy(PasteStrategy):
    ...
    def execute(self) -> bool:
        try:
            subprocess.run(["xdotool", "key", "--clearmodifiers", "ctrl+shift+v"], check=True, capture_output=True)
...
class XdotoolShiftInsertStrategy(PasteStrategy):
    ...
    def execute(self) -> bool:
        try:
            subprocess.run(["xdotool", "key", "--clearmodifiers", "shift+Insert"], check=True, capture_output=True)
...
class XdotoolTypeStrategy(PasteStrategy):
    ...
    def execute(self) -> bool:
        try:
            ...
            subprocess.run(
                ["xdotool", "type", "--clearmodifiers", "--delay", "10", content], check=True, capture_output=True
            )
...
class WtypeShiftInsertStrategy(PasteStrategy):
    ...
    def execute(self) -> bool:
        try:
            subprocess.run(["wtype", "-M", "shift", "-k", "Insert", "-m", "shift"], check=True, capture_output=True)
...
class WtypeDirectStrategy(PasteStrategy):
    ...
    def execute(self) -> bool:
        try:
            ...
            subprocess.run(["wtype", content], check=True, capture_output=True)
...
class WtypeCtrlVStrategy(PasteStrategy):
    ...
    def execute(self) -> bool:
        try:
            subprocess.run(["wtype", "-M", "ctrl", "-k", "v", "-m", "ctrl"], check=True, capture_output=True)
...
class PasteStrategyManager:
    ...
    def execute_paste(self, session_type: str, is_terminal: bool) -> bool:
        ...
        """
        applicable_strategies = [s for s in self.strategies if s.supports(session_type, is_terminal)]
