...
class TestDataGenerator:
    ...
    def generate_fragmented_samples(
        self, base_sentences: Optional[List[str]] = None, fragment_levels: List[str] = ["light", "moderate", "heavy"]
    ) -> Dict[str, List[Dict]]:
        ...
        for level in fragment_levels:
            ...
            for sentence in base_sentences:
                ...
                samples[level].append(
                    {"original": sentence, "fragmented": fragmented, "expected_merge": self._clean_sentence(sentence)}
                )
    ...
    def _fragment_moderately(self, sentence: str) -> str:
        ...
        for i, word in enumerate(words):
            if i > 0 and (
                word.lower() in self.conjunctions or word.lower() in self.prepositions or i == len(words) // 2
            ):
